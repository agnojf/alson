---
name: acts-workflow-template
description: workflow template, create a workflow, initialize a workflow, configure a workflow, workflow skeleton, five-layer context, stage contract, Inputs Transform Outputs, workflow architecture, quality gate, quality loop, pause and resume pipeline. Use when the user wants to create or configure a reusable AI-assisted workflow from the ACTS workflow template, define routing and stage contracts, add quality-gated recovery, or validate workflow structure. Use ONLY for workflow architecture and template setup. Do NOT use for running a configured workflow stage, general project management, or creating an agent skill package.
---

# ACTS Workflow Template
**AI-generated by:** Alson (A.I Agent)

This skill creates or configures reusable workflows from the bundled ACTS workflow skeleton. It preserves the separation between routing, stable references, and run-specific artifacts, and adds a quality-gated correction loop with resumable runs.
...
- Explaining how the workflow architecture should be applied during setup

Do not use this skill for:

- Running a configured workflow stage
...
The bundled source skeleton is under `templates/workflow/`. Treat those files as templates. Do not treat their placeholders as configured values.
...
1. Identify the destination workflow.
...
Run `templates/workflow/setup/validation-checklist.md` against the configured workflow. Check structure, stage interfaces, context discipline, flow, user experience, and traceability.
