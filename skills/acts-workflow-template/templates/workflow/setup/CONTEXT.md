...
Initialize the reusable skeleton into a configured workflow before any work stage runs.

## Inputs

| Source | File or location | Scope | Why |
|---|---|---|---|
| User | Setup answers and confirmed destination | Full response | Defines the workflow outcome, users, stages, inputs, outputs, and review rules |
...
1. Confirm the destination and the workflow outcome before writing.
2. Collect only the information needed to define the workflow identity, routing, source pointers, stage interfaces, outputs, review gates, and verification checks.
3. Populate the workflow-wide instructions and configuration without changing source material.
...
7. Record the setup decisions in `setup/initialization-record.md` in the initialized workflow.
...
|---|---|---|
| Configured workflow | User-confirmed destination | Folder with Markdown contracts and stage structure |
| Setup record | `setup/initialization-record.md` in the configured workflow | Markdown |
| Validation result | Chat or the configured workflow's setup output | Markdown |
