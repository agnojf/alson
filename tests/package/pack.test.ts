...
test('package: npm pack includes the CLI, catalog, and every skill', () => {
  ...
  assert.match(listing, /package\/skills\/acts-workflow-template\/SKILL\.md/);
  assert.match(listing, /package\/skills\/acts-workflow-template\/skill\.json/);
...
test('package: a fresh install of the tarball runs alson --help', () => {
  ...
  assert.match(list, /acts-workflow-template/);
